...
const Meal = ({ memberId }) => {
    ...
    const toggleMeal = async (mealId) => {
        ...
        if (isExpanding) {
            ...
            await journalStore.fetchMealNutrients({
                                    ...
                                    journalStore.meals[meal.id].map((item, index) => (
                                        <div key={index} className={styles.dishItem}>
                                            <div className={styles.dishAndGram}>
                                                ...
                                                <span className={styles.dishWeight}>
                                                    {item.weight} г
                                                </span>
                                            </div>
                                            <button
                                                ...
                                                onClick={async () => {
                                                    ...
                                                    await journalStore.fetchMealDishes(
                                                        meal.id,
                                                        memberIdToSend,
                                                    );
                                                    await journalStore.fetchMealNutrients({
                                                        memberId: memberIdToSend,
                                                        typeOfMealId: meal.id,
                                                    });
                                                    await journalStore.fetchConsumed({
                                                        userId: userIdToSend,
                                                        memberId: memberIdToSend,
                                                    });
                                                    window.location.reload();
