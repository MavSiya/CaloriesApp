...
class MenuService {
    ...
    async addDishOrIngredientToMenu({
        dayOfWeekId,
        typeOfMealId,
        userId,
        dishId = null,
    ...
    async deleteFromMenu(mpwDishId) {
        await db.pool.execute(`DELETE FROM MPW_Dish WHERE id = ?`, [mpwDishId]);
    ...
    async updateMenuItem(mpwDishId, dishId = null, ingredientId = null) {
        if (ingredientId !== null) {
            const [rows] = await db.pool.execute(`SELECT id FROM Ingredients WHERE id = ?`, [
                ingredientId,
            ]);
            if (rows.length === 0) {
