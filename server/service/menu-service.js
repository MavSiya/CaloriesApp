...
export async function addDishOrIngredientToMenu({
    ...
    if (menu.length > 0) {
        ...
        const [newMenu] = await db.execute(
            `
...
export async function deleteFromMenu(mpwDishId) {
    await db.execute(`DELETE FROM MPW_Dish WHERE id = ?`, [mpwDishId]);
...
export async function updateMenuItem(mpwDishId, dishId = null, ingredientId = null) {
    if (ingredientId !== null) {
        const [rows] = await db.execute(`SELECT id FROM Ingredients WHERE id = ?`, [ingredientId]);
        if (rows.length === 0) {
