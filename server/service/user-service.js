...
class UserService {
    async registration(email, password) {
        ...
        const [candidates] = await db.pool.query('SELECT * FROM users WHERE email = ?', [email]);
    ...
    async activate(activationLink) {
        const [users] = await db.pool.query('SELECT * FROM users WHERE activationLink = ?', [
            activationLink,
        ]);
        if (users.length === 0) {
        ...
        await db.pool.query('UPDATE users SET isActivated = true WHERE activationLink = ?', [
            activationLink,
        ]);
    }
    ...
    async refresh(refreshToken) {
        ...
        const [users] = await db.pool.query('SELECT * FROM users WHERE id = ?', [userData.id]);
        const user = users[0];
        const userDto = new UserDto(user);
        const tokens = tokenService.generateTokens({ ...userDto });
