...
class DishService {
    async getTypeIdByName(typeName) {
        ...
        const [rows] = await db.pool.execute('SELECT id FROM TypesOfDish WHERE title = ?', [
            typeName,
        ]);
    ...
    async getAllDishes(userId) {
        const [dishes] = await db.pool.execute('SELECT * FROM Dishes WHERE user_ID = ?', [userId]);
        return dishes;
    }
    ...
    async getAllDishTypes() {
        const [rows] = await db.pool.execute('SELECT * FROM TypesOfDish');
        return rows;
    }

    // dishService.js
    async getDishTypeById(typeId) {
        const [rows] = await db.pool.execute('SELECT * FROM TypesOfDish WHERE id = ?', [typeId]);
        return rows[0];
    }
}
