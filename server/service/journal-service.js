...
export async function getTypeOfMealIdByTitle(title) {
    if (!title) {
        throw new Error('Не вказано назву прийому їжі');
    }
    const [rows] = await db.execute('SELECT id FROM TypesOfMeal WHERE LOWER(title) = LOWER(?)', [
        title,
    ]);
...
export async function addDishToMeal({ userId, memberId, date, typeOfMealId, dishId, weight }) {
    ...
    if (journalEntry.length > 0) {
        ...
        const [newJournal] = await db.execute(
            `
    ...
    const [addedDish] = await db.execute(
        `
...
export async function addIngredientToMeal({
    ...
    if (journalEntry.length > 0) {
        ...
        const [dayOfWeekEntry] = await db.execute(
            `
    ...
    const [addedIngredient] = await db.execute(
        `
      INSERT INTO Journal_Dish (dish_ID, ingredient_ID, journal_ID, weight)
      VALUES (NULL, ?, ?, ?)
      `,
...
export async function calculateMealNutrients(userId, memberId = null, date, typeOfMealId) {
    let userCondition;
    let userValue;
...
export async function getMealByDateAndType({ date, typeOfMealId, userId, memberId }) {
    ...
    const [mealItems] = await db.execute(
        `
...
export async function getJournalDay({ userId, memberId, date }) {
    if (!userId && !memberId) {
    ...
    for (let mealType in groupedMeals) {
        ...
        const dishesInfo = await Promise.all(
            mealItems.map(async (meal) => {
                const [dishInfo] = await db.execute(
                    `SELECT title
           FROM Dishes
           WHERE id = ?`,
                    [meal.dishId],
                ...
                const [bmrInfo] = await db.execute(
